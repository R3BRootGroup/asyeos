...
// R3B headers
#include "R3BAsyTofDOnlineSpectra.h"
...
// R3BAsyTofDOnlineSpectra::Default Constructor --------------------------
R3BAsyTofDOnlineSpectra::R3BAsyTofDOnlineSpectra()
    : R3BAsyTofDOnlineSpectra("TofDOnlineSpectra", 1)
{
}

// R3BAsyTofDOnlineSpectra::Standard Constructor --------------------------
R3BAsyTofDOnlineSpectra::R3BAsyTofDOnlineSpectra(const TString& name, Int_t iVerbose)
    : FairTask(name, iVerbose)
{
}

// Virtual R3BAsyTofDOnlineSpectra::Destructor
R3BAsyTofDOnlineSpectra::~R3BAsyTofDOnlineSpectra()
{
...
void R3BAsyTofDOnlineSpectra::SetParContainers()
{
...
void R3BAsyTofDOnlineSpectra::SetParameter()
{
...
InitStatus R3BAsyTofDOnlineSpectra::Init()
{
    R3BLOG(info, "");
    ...
    if (fMappedItems && fCalItems)
    {
        ...
        for (Int_t j = 0; j < fNofPlanes; j++)
        {
            ...
            fh_tofd_channels[j] = R3B::root_owned<TH1F>(strName1, strName2, 2 * numPaddles, -numPaddles, numPaddles);
            ...
            fh_tofd_TotPm[j] = R3B::root_owned<TH2F>(
                strName3, strName4, 2 * numPaddles, -numPaddles, numPaddles, 3 * hrange, 0., hrange);
            ...
            fh_tofd_TotPm_coinc[j] = R3B::root_owned<TH2F>(
                strName5, strName6, 2 * numPaddles, -numPaddles, numPaddles, 3 * hrange, 0., hrange);
            ...
            fh_tofd_multihit[j] =
                R3B::root_owned<TH2F>(strName7, strName8, 2 * numPaddles, -numPaddles, numPaddles, 30, 0, 30);
            ...
            fh_tofd_multihit_coinc[j] =
                R3B::root_owned<TH2F>(strName9, strName10, numPaddles, 0., numPaddles, 30, 0, 30);
            ...
            fh_num_side[j] =
                R3B::root_owned<TH2F>(strName13, strName14, numPaddles, 0., numPaddles, numPaddles, 0, numPaddles);
            ...
            fh2_tofd_ypos_cal[j] =
                R3B::root_owned<TH2F>(strName13, strName14, numPaddles, 0, numPaddles, 2000., -50., 50.);
            ...
            fh2_tofd_timedif_cal[j] = R3B::root_owned<TH2F>(
                strName13, strName14, 2 * numPaddles, -numPaddles, numPaddles, 2000., -5000., 1000.);
        ...
        auto* cTofd_top_diagnosis = new TCanvas("TofD_top_diagnosis", "TOFD top diagnosis", 10, 10, 1100, 1000);
        cTofd_top_diagnosis->Divide(6, 2);
        ...
        for (Int_t j = 0; j < 2; j++)
            ...
            {
                char strName15[255];
                sprintf(strName15, "tofd_ToT_diagnosis_plane_%d_paddle_%d_top", j + 1, 20 + p);
                char strName16[255];
                ...
                fh_tofd_TotPm_top_vs_event[j * 6 + p]->GetXaxis()->CenterTitle(true);
                fh_tofd_TotPm_top_vs_event[j * 6 + p]->GetYaxis()->CenterTitle(true);
                fh_tofd_TotPm_top_vs_event[j * 6 + p]->GetXaxis()->SetLabelSize(0.045);
                fh_tofd_TotPm_top_vs_event[j * 6 + p]->GetXaxis()->SetTitleSize(0.045);
                fh_tofd_TotPm_top_vs_event[j * 6 + p]->GetYaxis()->SetLabelSize(0.045);
                fh_tofd_TotPm_top_vs_event[j * 6 + p]->GetYaxis()->SetTitleSize(0.045);
                cTofd_top_diagnosis->cd(j * 6 + p + 1);
                fh_tofd_TotPm_top_vs_event[j * 6 + p]->Draw("colz");
            }
        maintofd->Add(cTofd_top_diagnosis);
        ...
        for (Int_t j = 0; j < 2; j++)
            ...
            {
                char strName15[255];
                sprintf(strName15, "tofd_ToT_diagnosis_plane_%d_paddle_%d_bottom", j + 1, 20 + p);
                char strName16[255];
                ...
                fh_tofd_TotPm_bot_vs_event[j * 6 + p]->GetXaxis()->CenterTitle(true);
                fh_tofd_TotPm_bot_vs_event[j * 6 + p]->GetYaxis()->CenterTitle(true);
                fh_tofd_TotPm_bot_vs_event[j * 6 + p]->GetXaxis()->SetLabelSize(0.045);
                fh_tofd_TotPm_bot_vs_event[j * 6 + p]->GetXaxis()->SetTitleSize(0.045);
                fh_tofd_TotPm_bot_vs_event[j * 6 + p]->GetYaxis()->SetLabelSize(0.045);
                fh_tofd_TotPm_bot_vs_event[j * 6 + p]->GetYaxis()->SetTitleSize(0.045);
                cTofd_bot_diagnosis->cd(j * 6 + p + 1);
    ...
    if (fHitItems)
    {
        ...
        for (Int_t j = 0; j < fNofPlanes; j++)
        {
            ...
            fh_tofd_Tot_hit[j] = R3B::root_owned<TH2F>(strName3,
                                                       strName4,
                                                       numPaddles,
                                                       0,
                                                       numPaddles,
                                                       (j < 2 ? fMax_charge : fMax_chargeVeto) * 10,
                                                       0.,
                                                       (j < 2 ? fMax_charge : fMax_chargeVeto));
            ...
            fh_tofd_bars[j] = R3B::root_owned<TH1F>(strName21, strName22, numPaddles, 0., numPaddles);
        ...
        for (Int_t i = 0; i < fNofPlanes; i++)
        {
            ...
            fh_tofd_time_los_h2[i] = R3B::root_owned<TH2F>(strNameLos_c, strNameLos_c, 44, 1, 45, 20000, 0, 190);
            ...
            for (Int_t j = 0; j < fPaddlesPerPlane; j++)
            {
                ...
                fh_tofd_time_los[j][i] = R3B::root_owned<TH1F>(strNameLos, strNameLos2, 20000, 0, 190);
        ...
        fh2_tofd_time_vs_charge = R3B::root_owned<TH2F>(
            "fh2_tofd_time_vs_charge", "Time vs Charge", 10000, 0, 150, fMax_charge * 10, 0, fMax_charge);
        ...
        fh2_tofd_time_wouttrig_vs_charge = R3B::root_owned<TH2F>(
            "fh2_tofd_time_vs_charge_withoutTrig", "Time vs Charge", 10000, 0, 140, fMax_charge * 10, 0, fMax_charge);
        ...
        fh2_tofd_charges13 = R3B::root_owned<TH2F>("fh2_tofd_charge1_vs_charge3",
                                                   "Charges: Plane1 vs Plane3",
                                                   fMax_charge * 10,
                                                   0,
                                                   fMax_charge,
                                                   fMax_chargeVeto * 10,
        ...
        fh2_tofd_charges24 = R3B::root_owned<TH2F>("fh2_tofd_charge2_vs_charge4",
                                                   "Charges: Plane2 vs Plane4",
                                                   fMax_charge * 10,
                                                   0,
                                                   fMax_charge,
                                                   fMax_chargeVeto * 10,
        ...
        fh2_tofd_charges12 = R3B::root_owned<TH2F>("fh2_tofd_charge1_vs_charge2",
                                                   "Charges: Plane1 vs Plane2",
                                                   fMax_charge * 10,
                                                   0,
                                                   fMax_charge,
                                                   fMax_charge * 10,
        ...
        fh2_tofd_charges23 = R3B::root_owned<TH2F>("fh2_tofd_charge2_vs_charge3",
                                                   "Charges: Plane2 vs Plane3",
                                                   fMax_charge * 10,
                                                   0,
                                                   fMax_charge,
                                                   fMax_chargeVeto * 10,
        ...
        fh2_tofd_charges34 = R3B::root_owned<TH2F>("fh2_tofd_charge3_vs_charge4",
                                                   "Charges: Plane3 vs Plane4",
                                                   fMax_chargeVeto * 10,
                                                   0,
                                                   fMax_chargeVeto,
                                                   fMax_chargeVeto * 10,
        ...
        fh1_Zcharge =
            R3B::root_owned<TH1F>("fh1_tofd_chargeZ", "Charges Z from Plane 1", fMax_charge * 10, 0, fMax_charge);
        ...
        fh2_zcharge_tpat = R3B::root_owned<TH2F>(
            "fh2_Tofd_chargeZ_vs_tpat", "Charges Z vs tpat", 17, -0.5, 16.5, fMax_charge * 10, 0, fMax_charge);
...
void R3BAsyTofDOnlineSpectra::Reset_Histo()
{
    R3BLOG(info, "");
...
void R3BAsyTofDOnlineSpectra::Exec(Option_t* option)
{
    ...
    if (fHitItems)
    {
        ...
        if (charges[0] > 0 && charges[1] > 0)
        {
            ...
            if (charges[1] > 2.)
            {
                fh1_Zcharge->Fill(charges[1]);
                for (const auto& itpat : tpatindex)
                    fh2_zcharge_tpat->Fill(itpat, charges[1]);
            }
        }
...
void R3BAsyTofDOnlineSpectra::FinishEvent()
{
...
void R3BAsyTofDOnlineSpectra::FinishTask()
{
    if (fCalItems)
    {
        for (Int_t i = 0; i < fNofPlanes; i++)
...
ClassImp(R3BAsyTofDOnlineSpectra)
