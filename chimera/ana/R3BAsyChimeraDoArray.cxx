...
InitStatus R3BAsyChimeraDoArray::Init()
{
    ...
    if (verbose)
        LOG(info) << "R3BAsyChimeraDoArray::Init line 72";

    // --- ------------------------------------- --- //
    // --- get access to mapped data of CHIMERA  --- //
    // --- ------------------------------------- --- //
    if (verbose)
        LOG(info) << "R3BAsyChimeraDoArray::Init line 80";
    ...
    if (verbose)
        LOG(info) << "R3BAsyChimeraDoArray::Init line 87";

    // Register output array in tree
    fChimeraArrayData = new TClonesArray("R3BAsyChimeraArrayData");
    FairRootManager::Instance()->Register("ChimeraArrayData", "AsyChimera Array data", fChimeraArrayData, !fOnline);
...
void R3BAsyChimeraDoArray::Exec(Option_t* option)
{
    ...
    if (iMulti > 0)
    {
        ...
        for (int i = 0; i < dmaxs; i++)
        {
            if (E1F[i] > 0 && E2F[i] > 0 && T1F[i] >= TT && T2F[i] >= TT && (!uE1F[i]) && (!uE2F[i]))
            {
        ...
        {
            if (E1F[i] > 0 && E2F[i + 1] > 0 && T1F[i] >= TT && T2F[i + 1] >= TT && (!uE1F[i]) && (!uE2F[i + 1]))
            {
    ...
    if (iMulti > 0)
    {
        AddHitData(iMulti, iDet, iSide, iStrip, iRawEnergy, iRawTime, multE12, multEFB1);
    }
    else
    {
...
R3BAsyChimeraArrayData* R3BAsyChimeraDoArray::AddHitData(UInt_t multi,
                                                         UInt_t* Det,
                                                         UInt_t* Side,
                                                         UInt_t* Strip,
                                                         UInt_t* RawEnergy,
