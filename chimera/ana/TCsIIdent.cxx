...
void TCsIIdent::CsI_Identification(Int_t Numtel, Float_t fastpg, Float_t slowpg, TCHIResult *chiresult, float tave)
{
 ...
 //the grid does not exist
  ...
  if(izt==4 && iat==8)chiresult->Setpid(2.4);
        
}



void TCsIIdent::CsI_Identification_Base(TCsIParams *param, float fastp, float slowp,
                                        int *icods, int *izts, float *as)
{
 ...
 *icods = 0;
 ...
 for(int k=0; k<ln; k++) {
  ...
  if(slowp >= x1 && slowp < x2) {
   ...
   else if(ksup == -1) {          //bug corrected 10/2016
    ksup = k;
    dsup = -dist;
   }
   else {
 ...
 ibif = 0;
 if(ksup >= 0) {                 //bug corrected 10/2016
// point between two lines 
  if(kinf >= 0) {                 //bug corrected 11/2016
  ki = iz[ksup] - iz[kinf];
  dt = dinf + dsup;
// same Z 
   if(ki == 0) {
...
// Superior mass 
  if(ibif == 1 || ibif == 3) {
   ...
   if(it < ln && iz[it] == izt) {
    ...
    x1 = cx[ptrgap[it]];
    x2 = cx[ptrgap[j]-1];
    if(slowp < x1 || slowp > x2) icode = icode+1;
   }
  }
// Inferior masse  
  if(ibif == 2 || ibif == 3) {
   ...
   if(it>=0 && iz[it] == izt && it<ln) {
    x1 = cx[ptrgap[it]];
    x2 = cx[ptrgap[ki]-1];
    if(slowp < x1 || slowp > x2) icode = icode+2;
   }
  }
 }
