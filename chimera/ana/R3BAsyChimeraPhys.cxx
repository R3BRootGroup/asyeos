...
const Float_t thetamin[35] = {1.,    1.80,  2.60,  3.60,  4.60,  5.80,  7.00,
                              8.50,  10.00, 11.50, 13.00, 14.50, 16.00, 18.00,
                              20.00, 22.00, 24.00, 27.00, 30.,   38.,   46.,
                              54.,   62.,   70.,   78.,   86.,   94.,   102.,
                              110.,  118.,  126.,  134.,  142.,  150.,  163.};
const Float_t thetamax[35] = {1.80,  2.60,  3.60,  4.60,  5.80,  7.00,  8.50,
                              10.00, 11.50, 13.00, 14.50, 16.00, 18.00, 20.00,
                              22.00, 24.00, 27.00, 30.,   38.,   46.,   54.,
                              62.,   70.,   78.,   86.,   94.,   102.,  110.,
                              118.,  126.,  134.,  142.,  150.,  163.,  176.};
const Int_t telmin[35] = {0,   16,   32,   56,   80,   112,  144,  184, 224,
                          264, 304,  352,  400,  448,  496,  544,  592, 640,
                          688, 720,  752,  784,  816,  848,  880,  912, 944,
                          976, 1008, 1040, 1072, 1104, 1136, 1168, 1184};
const Int_t telmax[35] = {15,   31,   55,   79,   111,  143,  183,  223, 263,
                          303,  351,  399,  447,  495,  543,  591,  639, 687,
                          719,  751,  783,  815,  847,  879,  911,  943, 975,
...
R3BAsyChimeraPhys::~R3BAsyChimeraPhys() {
  ...
  if (fChimeraPhysData) delete fChimeraPhysData;
}
...
InitStatus R3BAsyChimeraPhys::Init() {
  ...
  if (verbose) LOG(info) << "R3BAsyChimeraPhys::Init line 72";
  // Register output array in tree
  fChimeraPhysData = new TClonesArray("R3BAsyChimeraPhysData");
  FairRootManager::Instance()->Register(
      "AsyChimeraPhysData", "AsyChimera Phys data", fChimeraPhysData, !fOnline);
  ...
  // --- ------------------------------------- --- //
  if (verbose) LOG(info) << "R3BAsyChimeraPhys::Init line 80";

  fMappedItemsChimera = (TClonesArray*)mgr->GetObject("AsyChimeraMappedData");
  if (!fMappedItemsChimera) {
    //        return kFATAL;
  }
  if (verbose) LOG(info) << "R3BAsyChimeraPhys::Init line 87";

  c_CHIMERA_phys =
      new TCanvas("c_CHIMERA_phys", "CHIMERA_phys", 0, 0, 1200, 1200);
  fh1_CHIMERA_multi =
      new TH1I("fh1_CHIMERA_multi", "CHIMERA_multi", 100, -0.5, 99.5);
  fh1_CHIMERA_RP = new TH1F("fh1_CHIMERA_RP", "CHIMERA_RP", 200, -200, 200);
  fh1_CHIMERA_RP12 =
      new TH1F("fh1_CHIMERA_RP12", "CHIMERA_RP12", 400, -400, 400);
...
void R3BAsyChimeraPhys::Exec(Option_t* option) {
  ...
  if (fMappedItemsChimera && fMappedItemsChimera->GetEntriesFast()) {
    ...
    for (Int_t ihit = 0; ihit < nHits; ihit++) {
      ...
      if (iFastLG > 0 && iTimeCsI > 0) {
        ...
        Float_t qx =
            sin(theta * TMath::DegToRad()) * cos(phi * TMath::DegToRad());
        Float_t qy =
            sin(theta * TMath::DegToRad()) * sin(phi * TMath::DegToRad());
  ...
  if (multi >= 8) {
    ...
    if (dmm < 0.33333) fh1_CHIMERA_RP12->Fill(DCHIRP12);
...
R3BAsyChimeraPhysData* R3BAsyChimeraPhys::AddPhysData(Float_t multi,
                                                      Float_t CHIRP) {
