   ...
   if(fA==0){
    if(de < 222.1 && de>0){
     for (iii=0;iii<70;iii++){
      ...
      if(de>DE11[69])erec=de+1;
     }
     idr->SetA(1);fA=1;
     stopped=0; 
     idr->SetUnStopped();
     if(idr->Getcod()<3)idr->Setcod(kICODE3);//ok...tbc
    }
...
//     if(myflag==1)idr->Setcod(kICODE5);
  ...
  if(fZ==3){
   ...
   if(fA==8){
    if(de>1899*tollup){// mettere valore esatto di PT
  ...
  if(fZ==4){
   if(fA==7){
    if(de>2573.71*tollup && de < 2827.02*tollup){// mettere valore esatto di PT
     idr->Setcod(kICODE4);//Z4A7->Z4A9
     idr->SetA(9);fA=9;
    }  
    if(de> 2827.02*tollup){// mettere valore esatto di PT
