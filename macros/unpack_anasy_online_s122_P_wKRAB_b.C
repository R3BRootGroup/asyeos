...
int unpack_anasy_online_s122(int nev)
{
    ...
    TString ucesb_path = ucesb_dir + "/../upexps/asyeos_gsi_s122/asyeos_s122 --input-buffer=90Mi";
    ...
    auto* source = new R3BUcesbSource(inputfile, ntuple_options, ucesb_path, &ucesb_struct, sizeof(ucesb_struct));
    source->SetMaxEvents(nev); // until contr-c

    // Adding readers here
    // auto unpackReader = new R3BUnpackReader((EXT_STR_h101_unpack_t*)&ucesb_struct.unpack, offsetof(EXT_STR_h101,
    // unpack)); source->AddReader(unpackReader);
    ...
    source->AddReader(new R3BAsyChimeraReader((EXT_STR_h101_ASYCHIMERA_t*)&ucesb_struct.asychimera,
                                              offsetof(EXT_STR_h101, asychimera)));
    source->AddReader(new R3BWhiterabbitMasterReader(&ucesb_struct.wrmaster, offsetof(EXT_STR_h101, wrmaster), 0x1000));
    source->AddReader(new R3BAsyKrabReader((EXT_STR_h101_ASYKRAB_t*)&ucesb_struct.asykrab,
                                           offsetof(EXT_STR_h101, asykrab),
                                           "/home/pcget/R3BRoot/asyeos/krab/pars/TBOX-KRAB.map"));
