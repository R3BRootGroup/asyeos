...
R3BAsyKrabReader::R3BAsyKrabReader(EXT_STR_h101_ASYKRAB* data, size_t offset,
...
Bool_t R3BAsyKrabReader::Init(ext_data_struct_info* a_struct_info) {
  ...
  EXT_STR_h101_ASYKRAB_ITEMS_INFO(ok, *a_struct_info, fOffset,
                                  EXT_STR_h101_ASYKRAB, 0);
  if (!ok) {
  ...
  FairRootManager::Instance()->Register(
      "AsyKrabMappedData", "AsyKrab mapped data", fArray, !fOnline);
  fArray->Clear();
  if (!fSkiptriggertimes) {
    FairRootManager::Instance()->Register("AsyKrabTriggerMapped",
                                          "AsyKrab trigger mapped data",
                                          fArrayTrigger, DoNotWrite);
  } else {
    fArrayTrigger = nullptr;
  }

  FairRootManager::Instance()->Register("AsyKrabWRData", "WRKrab", fArrayWR,
                                        !fOnline);
...
Bool_t R3BAsyKrabReader::R3BRead() {
  ...
  timestamp = ((uint64_t)fTIMESTAMP_KRAB_WR_T4 << 48) |
  ...
  if (timestamp == 0) {
    new ((*fArray)[fArray->GetEntriesFast()])
        R3BAsyKrabMappedData(0, 0, 0, 0, 0);
  } else {
    for (int i = 0; i < 32; i++) {
      auto const& test = data->KRAB_DATAv[i];
      if (verbose > 1) std::cout << std::hex << test << std::dec << " ";
      if (verbose > 1 && i == 31) std::cout << std::endl;
    }
    if (verbose > 1) {
      std::cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@" << std::endl;
      std::cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@" << std::endl;
    ...
    int icorr[780] = {0};
    for (int ii = 0; ii < 780; ii++) icorr[ii] = 0;
    int imultk = 0;
    int icorrk[780] = {0};
    for (int ii = 0; ii < 780; ii++) icorrk[ii] = 0;

    const int HSIZE = 0;  // header+timestamps size

    for (int ichtb = 0; ichtb < 30; ichtb++) {  // TBox FPGAs
        ...
        for (int bit = 0; bit < 26; bit++) {
          if (pattern & (1 << bit)) {
            ...
            if (xtbox[detid] > -1) {
              ...
              multr[rng]++;
              multr[5]++;
              if (rng == 1 && chrng == 25) itest = 1;
              new ((*fArray)[fArray->GetEntriesFast()])
                  R3BAsyKrabMappedData(rng, chrng, ang, qx, qy);
            }
          }
        }
      }
