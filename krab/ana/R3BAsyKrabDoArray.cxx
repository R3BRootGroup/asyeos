...
InitStatus R3BAsyKrabDoArray::Init() {
  ...
  if (verbose) LOG(info) << "R3BAsyKrabDoArray::Init line 72";

  // --- ------------------------------------- --- //
  // --- get access to mapped data of KRAB     --- //
  // --- ------------------------------------- --- //
  if (verbose) LOG(info) << "R3BAsyKrabDoArray::Init line 80";

  fMappedItemsKrab = (TClonesArray*)mgr->GetObject("AsyKrabMappedData");
  if (!fMappedItemsKrab) {
    return kFATAL;
  }
  if (verbose) LOG(info) << "R3BAsyKrabDoArray::Init line 87";

  // Register output array in tree
  fKrabArrayData = new TClonesArray("R3BAsyKrabArrayData");
  FairRootManager::Instance()->Register("KrabArrayData", "AsyKrab Array data",
                                        fKrabArrayData, !fOnline);
...
void R3BAsyKrabDoArray::Exec(Option_t* option) {
  ...
  if (fMappedItemsKrab && fMappedItemsKrab->GetEntriesFast()) {
    ...
    for (Int_t ihit = 0; ihit < nHits; ihit++) {
      ...
      if (hitmapped->GetRing() >= 0 && hitmapped->GetSector() >= 0) {
        ...
        if (iMulti1 >= nHits / 2) rrr = 0.25;
        if (iMulti2 >= nHits / 2) rrr = 0.75;
  ...
  */
  if (iMulti > 1) {
    ...
    if (verbose) std::cout << " iRP= " << iRP << std::endl;
  ...
  nn++;
  if (verbose)
    LOG(info) << "R3BAsyKrabDoArray: " << fKrabArrayData->GetEntriesFast()
...
R3BAsyKrabArrayData* R3BAsyKrabDoArray::AddHitData(UInt_t Multi, Double_t RP,
                                                   UInt_t* Ring, UInt_t* Sector,
                                                   Float_t* Phi) {
  TClonesArray& clref = *fKrabArrayData;
  Int_t size = clref.GetEntriesFast();
  if (verbose) LOG(info) << "Add Hit Data....";
