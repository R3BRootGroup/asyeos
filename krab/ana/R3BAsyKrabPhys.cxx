...
R3BAsyKrabPhys::~R3BAsyKrabPhys() {
  ...
  if (fKrabPhysData) delete fKrabPhysData;
}
...
InitStatus R3BAsyKrabPhys::Init() {
  ...
  if (verbose) LOG(info) << "R3BAsyKrabPhys::Init line 72";
  // Register output array in tree
  fKrabPhysData = new TClonesArray("R3BAsyKrabPhysData");
  FairRootManager::Instance()->Register("AsyKrabPhysData", "AsyKrab Phys data",
                                        fKrabPhysData, !fOnline);
  ...
  // --- ------------------------------------- --- //
  if (verbose) LOG(info) << "R3BAsyKrabPhys::Init line 80";

  fMappedItemsKrab = (TClonesArray*)mgr->GetObject("AsyKrabMappedData");
  if (!fMappedItemsKrab) {
    //        return kFATAL;
  }
  if (verbose) LOG(info) << "R3BAsyKrabPhys::Init line 87";
