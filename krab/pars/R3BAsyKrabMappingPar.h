...
class R3BAsyKrabMappingPar : public FairParGenericSet
{
  public:
    ...
    const Int_t GetBoxRingCh(Int_t sensor) { return fxtboxringch->GetAt(sensor - 1); }
    const Float_t GetBoxAng(Int_t sensor) { return fxtboxang->GetAt(sensor - 1); }
    const Int_t GetBoxBoard(Int_t sensor) { return fxtboxboard->GetAt(sensor - 1); }
    ...
    const Int_t GetBoxFpgaCh(Int_t sensor) { return fxtboxfpga->GetAt(sensor - 1); }
    const Int_t GetBoxCrock(Int_t sensor) { return fxtboxcrock->GetAt(sensor - 1); }
    const Int_t GetBoxCrockCh(Int_t sensor) { return fxtboxcrockch->GetAt(sensor - 1); }

    void Setnumsectors(Int_t numSectors) { fNumSectors = numSectors; }
    void SetBoxRing(Int_t value, Int_t sensor) { fxtboxring->AddAt(value, sensor - 1); }
    void SetBoxCh(Int_t value, Int_t sensor) { fxtboxch->AddAt(value, sensor - 1); }
    void SetBox(Int_t value, Int_t sensor) { fxtbox->AddAt(value, sensor - 1); }
    void SetBoxRingCh(Int_t value, Int_t sensor) { fxtboxringch->AddAt(value, sensor - 1); }
    void SetBoxAng(Float_t value, Int_t sensor) { fxtboxang->AddAt(value, sensor - 1); }
    void SetBoxBoard(Int_t value, Int_t sensor) { fxtboxboard->AddAt(value, sensor - 1); }
    void SetBoxInp(Int_t value, Int_t sensor) { fxtboxinp->AddAt(value, sensor - 1); }
    void SetBoxFpga(Int_t value, Int_t sensor) { fxtboxfpga->AddAt(value, sensor - 1); }
    void SetBoxFpgaCh(Int_t value, Int_t sensor) { fxtboxfpga->AddAt(value, sensor - 1); }
    void SetBoxCrock(Int_t value, Int_t sensor) { fxtboxcrock->AddAt(value, sensor - 1); }
    void SetBoxCrockCh(Int_t value, Int_t sensor) { fxtboxcrockch->AddAt(value, sensor - 1); }
  ...
  private:
    ...
    */
    ...
    const R3BAsyKrabMappingPar& operator=(const R3BAsyKrabMappingPar&); /*< an assignment operator>*/
