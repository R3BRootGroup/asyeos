...
void R3BAsyKrabContFact::setAllContainers()
{
    ...
    FairContainer* p1 = new FairContainer("KrabCalPar", "Krab Cal Parameters", "KrabCalParContext");
    p1->addContext("KrabCalParContext");
    containers->Add(p1);

    FairContainer* p2 = new FairContainer("KrabHitPar", "Krab Hit Parameters", "KrabHitParContext");
    p2->addContext("KrabHitParContext");
    containers->Add(p2);

    FairContainer* p3 = new FairContainer("KrabGeoPar", "Krab geometry parameters", "GeometryParameterContext");
    p3->addContext("GeometryParameterContext");
    containers->Add(p3);

    FairContainer* p4 = new FairContainer("KrabMappingPar", "Krab mapping parameters", "KrabMappingContext");
...
FairParSet* R3BAsyKrabContFact::createContainer(FairContainer* c)
{
    ...
    {
        p = new R3BAsyKrabCalPar(c->getConcatName().Data(), c->GetTitle(), c->getContext());
    }

    if (strcmp(name, "KrabHitPar") == 0)
    {
        p = new R3BAsyKrabHitPar(c->getConcatName().Data(), c->GetTitle(), c->getContext());
    }
