...
R3BAsyKrabMappingPar::R3BAsyKrabMappingPar(const char* name, const char* title,
                                           const char* context)
    : FairParGenericSet(name, title, context), fNumSectors(736) {
...
R3BAsyKrabMappingPar::~R3BAsyKrabMappingPar() {
  if (fxtboxring) delete fxtboxring;
  if (fxtboxch) delete fxtboxch;
  if (fxtbox) delete fxtbox;
  if (fxtboxringch) delete fxtboxringch;
  if (fxtboxang) delete fxtboxang;
  if (fxtboxboard) delete fxtboxboard;
  if (fxtboxinp) delete fxtboxinp;
  if (fxtboxfpga) delete fxtboxfpga;
  if (fxtboxfpgach) delete fxtboxfpgach;
  if (fxtboxcrock) delete fxtboxcrock;
  if (fxtboxcrockch) delete fxtboxcrockch;
}
...
void R3BAsyKrabMappingPar::printParams() {
  ...
  for (Int_t i = 0; i < fNumSectors; i++) {
    LOG(info) << i + 1 << " " << fxtboxring->GetAt(i) << " "
              << fxtboxch->GetAt(i) << " " << fxtbox->GetAt(i) << " "
              << fxtboxringch->GetAt(i) << " " << fxtboxang->GetAt(i) << " "
              << fxtboxboard->GetAt(i) << " " << fxtboxinp->GetAt(i) << " "
              << fxtboxfpga->GetAt(i) << " " << fxtboxfpgach->GetAt(i) << " "
